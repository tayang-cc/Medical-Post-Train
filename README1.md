...
```
reward = 格式门控 × ( result_weight × 二进制结果 + process_weight × min(PRM 步分) )
```

- **格式门控**：最终答案可解析才通过，否则 reward=0（防格式劫持，与结果奖励解耦）。
- **二进制结果**：抽取答案 == 标准答案（0/1）。
- **过程奖励**：整条轨迹分步打分后取 **min**（最薄弱一步决定过程质量）。
